...
void logArray(const NSArray *array)
{
  NSEnumerator *iter;
  id arg;
...
NSString *pathToProjectFromArguments(BOOL *ok) 
{
  NSArray* arguments;
  NSEnumerator *iter;
  BOOL hasProject = NO;
  ...
  BOOL dir, exists;
  exists = [[NSFileManager defaultManager] fileExistsAtPath: result isDirectory: &dir];
  *ok = (exists == YES && dir == YES) ? YES : NO;
  return result;
}


BOOL isLibrary(NSString *folderPath, NSString *name)
{
  ...
  exists = [[NSFileManager defaultManager]
                fileExistsAtPath: fullName isDirectory: &dir]; 
  return (exists == YES && dir == NO) ? YES : NO; 
}


void printLibrary(NSString *path)
{
  NSArray *names;
  NSMutableArray *dbs;
  NSString *name;
